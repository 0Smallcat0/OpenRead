...
src/
  ...
  ui/
    ...
    epub.ts               container.xml → package file → spine, contents
    epub-render.ts        chapter XHTML → DOM, with script and the network out
    epub-reader.ts        the reader: chapters, contents, remembered position
  ...
  entrypoints/
    background.ts         service worker: stream broker + PDF/EPUB router + enrich
...
| The EPUB reader scrolls; it does not paginate                                   | Pagination means owning the layout, and whole-page translation changes the height of a chapter as it is translated — so a paginator would re-break the whole book underneath the reader each time a translation landed. A scrolling document grows instead, which is why `blocks.ts`, `fullpage.ts` and `selection.ts` all work on this page with no reader-specific code.                                                                                                                                                                                                                                                                                                                                 |
| The chapter goes in `<main>`; the toolbar and contents stay outside it          | `collectBlocks` scopes a page to its content landmark, so the book is the content and the reader's own controls are excluded by construction rather than by blocklist. Get it wrong and a press translates "Contents", "Previous" and "Translate" while the reader waits on paragraph one.                                                                                                                                                                                                                                                                                                                                                                                                                 |
| A book's own stylesheets are dropped                                            | Publisher CSS assumes a paginated viewport it controls — fixed heights, absolute positioning, page-break rules — and in a scrolling page it produces overlapping text as often as the intended look. It would also be styling the translated lines this extension inserts, which it has never seen. Verified in `e2e:epub` with a fixture whose stylesheet says `display: none`, so a leak makes the chapter invisible rather than merely different.                                                                                                                                                                                                                                                       |
| A remote reference inside a chapter is dropped, not fetched                     | The specification requires everything a book needs to be in the archive, so a remote image is a request the reader did not make and a note to the publisher about which page they are on. `e2e:epub` records every request the reader page makes and fails on any that is not `chrome-extension:` or `blob:`.                                                                                                                                                                                                                                                                                                                                                                                              |
| A hand-written zip reader over `DecompressionStream`, not a zip library         | The browser already ships the hard half. What a library adds is the container format — end-of-central-directory, the file table, and the arithmetic to find each entry — which is fully specified and unit-testable against archives built in the test. Cheaper than another dependency in a Web Store package. The central directory is read rather than the local headers, because a streaming producer zeroes the local sizes and a reader that trusts them returns an empty chapter.                                                                                                                                                                                                                   |
